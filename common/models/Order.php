    ...
    public function addOrder($data)//添加订单
    {
        ...
        if(isset($data["telFeeDeductions"]))
        {
            ...
            /*  $mobileApi=new MobileApi();
             ...
             /* if($myUserTelFee<$telFeeDeductions&&$myUserTelFee>0)//话费少于可以抵扣金额并且大于0  
              ...
              {
                  if($myUserTelFee>=$telFeeDeductions)
                  {
                      /*if($myUserTelFee<$telFeeDeductions&&$myUserTelFee>0)//话费少于可以抵扣金额并且大于0  
                      {
                            $telFeeDeductions=$myUserTelFee;
                    ...
                    $reduceTelFare=$suibianda->callPay("-".$telFeeDeductions);
                      if(!$reduceTelFare)
                      {
                          echo "话费扣除失败";
                          die();
        ...
        try {
            ...
            if($telFeeDeductions!="0")
            {
                ...
                $user->get_call_fee=round($user->get_call_fee-$telFeeDeductions,2);
            ...
            $userCallfeeLog->type="2";
            $userCallfeeLog->user_id=$user_id;
            $userCallfeeLog->fee=$telFeeDeductions;
            $userCallfeeLog->card_num="";
            $userCallfeeLog->order_sn=$this->order_sn;
