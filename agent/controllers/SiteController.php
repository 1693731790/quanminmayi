...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $user=User::findOne($user_id);
      	$agent=Agent::findOne(["user_id"=>$user_id]);
      	if(empty($agent))
        {
          	Yii::$app->user->logout();
        ...
        $saleFee=AgentFeeRecord::find()->where(["user_id"=>$user_id])->sum("fee");//销售额
        
        $walletFee=Wallet::find()->where(["user_id"=>$user_id])->sum("fee");//钱包分红
		
        $agentNum=Agent::find()->where(["parent_id"=>$user_id])->count();//my代理商总人数
     ...
     public function actionLogin($return_url=null){
        ...
        if(Yii::$app->request->isPost){
            if ($model->load(Yii::$app->request->post())) {
                ...
                if($user=$model->login()){
                   $user_id=Yii::$app->user->identity->id;
                   $agent=Agent::find()->where(["user_id"=>$user_id,"ispay"=>"1"])->one(); 
                   //$this->dump($shop);
                   if(empty($agent))
                   {
                       // echo "123";
