...
{ 
    ...
    public function actionGoodsLowerframe($goods_id,$sale)//下架商品
  {
    ...
    public function actionView($id)
    {
        ...
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
		$supplier =Supplier::findOne($model->source_id);
    ...
    function actionShopCate($pid)
    {
        $user_id=Yii::$app->user->identity->id;
        $shop=Shops::findOne(["user_id"=>$user_id]);
       $region=ShopsClass::find()->asArray()->where(["shop_id"=>$shop->shop_id,"pid"=>$pid])->all();
         // $this->dump($region);
       ...
       // $this->dump($model->getErrors());
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['update', 'id' => $model->goods_id,"attr"=>"1"]);
          	  ...
          	  $shopclass=ShopsClass::find()->select(["id","pid","name"])->where(["shop_id"=>$shop->shop_id,"pid"=>0])->asArray()->all();
       //$this->dump($shopclass);
    ...
    {   
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
           ...
           $shopclass=ShopsClass::find()->select(["id","pid","name"])->where(["pid"=>0])->asArray()->all();
    ...
    {   
        ...
        if($model->shop_id==$shop->shop_id)
        {
            ...
            throw new NotFoundHttpException('没有操作权限');
        }
