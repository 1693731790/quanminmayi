...
$shopscate=ShopsCate::find()->select(["id","title"])->all();
...
<div class="tab-content" >
 ...
 <?php $form = ActiveForm::begin([
    ...
    <div class="form-group required">
        <div class="row">
            <div class="col-md-2 text-right"><label>系统商品分类</label></div>
            <div class="col-md-10">
  ...
  <div class="form-group">
        <div class="row">
            ...
            <div class="col-md-10">
                <span class="btn btn-success" onclick="brandselect()">选择系统品牌</span>
  ...
  <div class="form-group">
     ...
     <div class="form-group required">
        <div class="row">
            <div class="col-md-2 text-right"><label>店铺商品分类</label></div>
            <div class="col-md-10">
                <select id="shop_class1" name="Goods[shops_class1]">
                  <option>顶级分类</option>
                   <?php foreach($shopclass as $shopclassval):?>
                  	<option value="<?=$shopclassval['id']?>" <?=$shopclassval['id']==$model->shops_class1?"selected":""?>><?=$shopclassval['name']?></option>
                   <?php endforeach;?>
                </select>
                <select id="shop_class2" name="Goods[shops_class2]">
                  <option>二级分类</option>
                  <option value="<?=$model->shops_class2?>" selected><?=ShopsClass::getCateName($model->shops_class2)?></option>
                </select>
               
            <?=Html::error($model , 'shops_class1' , ['class' => 'error'])?>
            <?=Html::error($model , 'shops_class2' , ['class' => 'error'])?>
  ...
  <script type="text/javascript">
    $(function(){
    	    $("#shop_class1").change(function(){
        var pid=$(this).val();
      
        if(pid!="")
        {
            $.get("<?=Url::to(['goods/shop-cate'])?>",{"pid":pid},function(data){
                console.log(data);
                $("#shop_class2").empty();
      ...
      <div class="form-group required">
        <div class="row">
            <div class="col-md-2 text-right"><label>店铺品牌</label></div>
            <div class="col-md-10">
                <select id="shops_cate" name="Goods[shops_cate]">
                  <option>顶级分类</option>
                   <?php foreach($shopscate as $shopscate):?>
                  	<option value="<?=$shopscate['id']?>" <?=$shopscate['id']==$model->shops_cate?"selected":""?>><?=$shopscate['title']?></option>
                   <?php endforeach;?>
                </select>
               
            <?=Html::error($model , 'shops_cate' , ['class' => 'error'])?>
        ...
        <div class="row">
            ...
            <div class="col-md-10">
                <div style="border: 1px dashed #ccc;width:212px;height:152px;margin-bottom: 10px;">
                    <img  id="goods_thums" width="212" height="152"  src="<?=$model->isNewRecord?'/static/default.jpg':Yii::$app->params['imgurl'].$model->goods_thums?>" />  
                </div>
    ...
    <div class="row">
                ...
                <div class="jq_uploads_img" >
                   <?php if($model->goods_img):?>
                      <?php for($i=0;$i<count($model->goods_img);$i++):?>
                        <span style="width: 150px; height: 100px;float: left; margin-left: 5px; margin-top: 10px;">  <img width="80" height="80" src="<?=Yii::$app->params['imgurl'].$model->goods_img[$i]?>">  <input type="hidden" name="Goods[goods_img][]" value="<?=$model->goods_img[$i]?>" />    <a href="javascript:void(0);">取消</a>  </span>
                       <?php endfor;?>                
                    <?php endif;?>
                </div>
                 ...
                 <script>
                        ...
                        $("#photo_file_goods_thums").uploadify({
                            ...
                            'onUploadSuccess': function (file, data, response) {
                                ...
                                $("#goods_thums").attr('src', "<?=Yii::$app->params['imgurl']?>"+data).show();
         ...
         var str = '<span style="width: 150px; height: 100px; float: left; margin-left: 5px; margin-top: 10px;">  <img width="80" height="80" src="' + "<?=Yii::$app->params['imgurl']?>"+data + '">  <input type="hidden" name="Goods[goods_img][]" value="' + data + '" />    <a href="javascript:void(0);">取消</a>  </span>';
    ...
    <?= $form->field($model, 'stock')->textInput(['maxlength' => true]) ?>
      <?= $form->field($model, 'shops_limit')->checkBox() ?>
