...
class UserController extends CommonController
{
      ...
      public function actionIndex()
      {
        ...
        return $this->render("index",[
              ...
              "recharge_fee"=>$user->recharge_fee,
        ]);
    ...
    function actionOrderCancel($order_id)//用户取消订单
    {
        ...
        try {
            ...
            if(!$model->update(true,["status"]))
            {
          	...
          	$userAuth=UserAuth::find()->where(["identity_type"=>"phone","user_id"=>$user_id])->one();
          	//throw new \yii\db\Exception($userAuth->identifier);
            $suibianda=new Suibianda($userAuth->identifier);
          	if(!$suibianda->callPay($model->telfare_fee))
            {
             	 throw new \yii\db\Exception("保存失败");  
            }
          	...
          	$user=User::findOne($user_id);
          	$user->get_call_fee=round($user->get_call_fee+$model->telfare_fee,2);
            if(!$user->update(true,["get_call_fee"]))
            { 
             	throw new \yii\db\Exception("保存失败");  
            }
            $userCallfeeLog=new UserCallfeeLog();
            $userCallfeeLog->type="3";
            $userCallfeeLog->user_id=$user_id;
            $userCallfeeLog->fee=$model->telfare_fee;
            $userCallfeeLog->card_num="";
            $userCallfeeLog->order_sn=$model->order_sn;
            $userCallfeeLog->create_time=time();
            if(!$userCallfeeLog->save())
            { 
             	throw new \yii\db\Exception("保存失败");  
            }
    ...
    function actionRecharge()  //话费充值
    {
        ...
        if($data=Yii::$app->request->post()){
          	...
          	if(empty($callRechargeCard))
            {
          	...
          	if($callRechargeCard->end_time<time())
            { 
              	  $res['success']=false;
                  $res['message']="充值失败，此卡已过期";
                  return json_encode($res); 
            }
            ...
            if($suibianda->callPay($callRechargeCard->fee))
            {
                ...
                $userCallfeeLog=new UserCallfeeLog();
              	$userCallfeeLog->create($user_id,$callRechargeCard->fee,$callRechargeCard->card_num,$user->get_call_fee);
