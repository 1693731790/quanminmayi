...
<script type="text/javascript">
  $(function(){
      $("#submit").click(function(){
          ...
          if(check)
          {
              $.post("<?=Url::to(['user/order-refund'])?>",{"refund_remarks":refund_remarks,"order_id":<?=$order_id?>},function(r){
                  if(r.success)
                  {              
                      layer.msg(r.message);  
                      setTimeout(function(){window.location.href="<?=Url::to(['user/order','status'=>'4'])?>";  },2000);    
