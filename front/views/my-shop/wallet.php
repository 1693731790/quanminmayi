    ...
    <div class="CommissionList">
      <div class="CommissionTabs">
        <ul>
          ...
          <li><i class="iconfont icon-withdrawcashicon"></i>类型</li>
        </ul>
      </div>
      <div class="list f22">
        <ul id="list_box">
          <?php foreach($model as  $val):?>
          <li> <span class="tit pl30"><?=date("Y-m-d H:i:s",$val->create_time)?></span><span class="num1 pl18"><?=$val->fee?></span><span class="num2 pl20"><?=Yii::$app->params["wallet_type"][$val->type]?></span></li>
         <?php endforeach;?>
        </ul>
      </div>
    </div>
...
//加载数据
 function LoadingDataFn() {
    $.get("<?=Url::to(['my-shop/wallet-list'])?>",{"page":page},function(da){
             $('#list_box').append(da);                                      
    })
