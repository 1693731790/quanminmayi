            ...
            if($data['status']==1)
            {
                ...
                if($result)
                {
                    ...
                    if($user->user_parent_id!="")
                    {
                  	...
                  	if($order->telfare_fee!="0")
                    {
                      	$user->get_call_fee=round($user->get_call_fee+$order->telfare_fee,2);
                      	$user->update(true,["get_call_fee"]);
                        
                        $userAuth=UserAuth::find()->where(["identity_type"=>"phone","user_id"=>$order->user_id])->one();
                      
                      	$suibianda=new Suibianda($userAuth->identifier);
                        $suibianda->callPay($order->telfare_fee);
                      	
                      	$userCardLog=new UserCallfeeLog();
                      	
                      	$userCardLog->create($order->user_id,$order->telfare_fee,"",$user->get_call_fee,"3",$order->order_sn);
